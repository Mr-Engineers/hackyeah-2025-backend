...
class CareerService:
    ...
    def get_random_job_offers(self, db: AsyncSession) -> list:
        player_state = self.player_state_service.load_state()
        random_jobs = self.job_repo.get_random_jobs(db, limit=3)
        job_offers = []
    ...
    def apply_for_job(self, db: AsyncSession, job_id: int) -> None:
        player_state = self.player_state_service.load_state()
    ...
    def promotion_event(self, db: AsyncSession) -> bool:
        player_state = self.player_state_service.load_state()
        if not player_state or not player_state.job_id:
            return False
        current_job = self.job_repo.get_by_id(db, player_state.job_id)
        if not current_job:
            return False
        next_tier = current_job.tier + 1
        next_job = self.job_repo.find_job_by_title_and_tier(
            db,
    ...
    def promotion(self, db: AsyncSession) -> bool:
        player_state = self.player_state_service.load_state()
        if not player_state or not player_state.job_id:
            return False
        current_job = self.job_repo.get_by_id(db, player_state.job_id)
